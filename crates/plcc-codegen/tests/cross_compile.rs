...
fn compile_to_target(source: &str, triple: &str) {
    ...
    let safe_name: String = triple.chars().map(|c| if c.is_alphanumeric() { c } else { '_' }).collect();
    let obj_path = dir.join(format!("test_{safe_name}.o"));
    ...
    let meta = std::fs::metadata(&obj_path).unwrap_or_else(|e| panic!("object file not created at {}: {e}", obj_path.display()));
...
fn emit_x86_64_linux() {
    compile_to_target(BLINK_ST, "x86_64-unknown-linux-gnu");
}

#[test]
fn emit_x86_64_linux_state_machine() {
    compile_to_target(STATE_MACHINE_ST, "x86_64-unknown-linux-gnu");
}

#[test]
fn emit_aarch64_linux() {
    compile_to_target(BLINK_ST, "aarch64-unknown-linux-gnu");
}

#[test]
fn emit_armv7_none_eabi() {
    // Bare-metal ARM Cortex-M — typical MCU target
    compile_to_target(BLINK_ST, "armv7-unknown-none-eabi");
}

#[test]
fn emit_thumbv7em_none_eabi() {
    // ARM Cortex-M4/M7 — PLC-class MCU
    compile_to_target(BLINK_ST, "thumbv7em-unknown-none-eabi");
}

#[test]
fn emit_wasm32() {
    // WebAssembly — browser/web runtime
    compile_to_target(BLINK_ST, "wasm32-unknown-unknown");
}

#[test]
fn emit_wasm32_state_machine() {
    compile_to_target(STATE_MACHINE_ST, "wasm32-unknown-unknown");
}

#[test]
fn emit_riscv32() {
    // RISC-V 32-bit — emerging MCU arch
    compile_to_target(BLINK_ST, "riscv32-unknown-none-elf");
}
